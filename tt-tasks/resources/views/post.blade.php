        ...
        <div class="col-lg-4 col-sm-6">
            ...
            <div class="card card-outline-primary mb-2 shadow">
                <div class="card-header">
                    ...
                    <h6 class="badge text-light badge-success text-success mb-1">{{ $post->progress }}</h6>
                    @elseif (($post->progress) == "WIP")
                    <h6 class="badge text-light badge-warning text-warning mb-1">{{ $post->progress }}</h6>
                    @else
                    <h6 class="badge text-light badge-secondary text-secondary mb-1">Not picked up</h6>
                    @endif
                    <h5 class="card-title">{{ $post->priority }} | {{ $post->task }}</h5>
                </div>

                <div class="card-body">
                    <h6 class="card-subtitle mb-2">{{ $post->site }}</h6>
